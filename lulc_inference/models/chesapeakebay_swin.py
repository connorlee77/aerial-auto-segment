...
class SwinSegmentationModel(nn.Module):
    def __init__(self, img_size=256, in_channels=3, num_classes=7):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for a in feature_list:
            ...
            reshaped_feature_list.append(a.permute(0, 2, 1).view(B, C, size, size))

        x = self.decoder_head(reshaped_feature_list)
        return x
...
class FPN_fuse(nn.Module):
    def __init__(self, feature_channels=[256, 512, 1024, 2048], fpn_out=256):
        ...
        assert feature_channels[0] == fpn_out, 'The first feature channel must be equal to fpn_out: {} != {}'.format(
            feature_channels[0], fpn_out)
...
class UperNet(nn.Module):
    ...
    def __init__(self, img_size, num_classes, feature_channels=[256, 512, 1024, 1024], fpn_out=256, freeze_bn=False):
    ...
    def forward(self, features_list):
        features_list[-1] = self.PPN(features_list[-1])
        x = self.head(self.FPN(features_list))
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, features_only=False, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        x, feature_list = self.forward_features(x)
