...
def reproject_resample_rasters(label_raster_path, spatial_res, output_folder, place_name, data_path, force_reproject=False, save_raster_previews=False):
    """
    ...
    dataset_type_dict = {
        ...
        # TODO: Add dynamic world path once datasets are downloaded from earth engine
    ...
    for dataset_type, obj in dataset_type_dict.items():
        ...
        if not os.path.exists(reprojected_path) or force_reproject:
            ...
            if save_raster_previews:
                save_raster_preview_as_png(reprojected_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--place_name', type=str, help='Name of place (castaic lake, colorado river, etc...)',
                        choices=['castaic_lake', 'colorado_river', 'big_bear_lake', 'duck', 'kentucky_river'])
    parser.add_argument('--label_raster_path', type=str,
                        help='Filepath to DynamicWorld label raster for the given place.')
    ...
    # TODO: Create filepath for dynamic world labels
    label_raster_path = args.label_raster_path
    reproject_resample_rasters(label_raster_path, args.spatial_res,
